...
def deploy_dir(target_address_deploed, target_address_downloaded):
    ...
    try:
        ...
        if os.path.exists(target_address_downloaded):
            ...
            if os.path.isfile(target_address_deploed):
                os.system('copy "' + target_address_deploed + '" "' + target_address_downloaded + '"')
                # os.system('echo Y | copy "'+target_address_deploy+ '" "' + target_address_downloaded + '"')
            else:
                shutil.copytree(convert_path_style(target_address_deploed, "1"), convert_path_style(target_address_downloaded, "1"))
            AI_speak("directory 대체를 완료했습니다")
            # os.system("bandizip.exe c "+target_address_deploy+" - "+yyyy_MM_dd_HH_mm_ss+" "+target_address_deploy+"")
        else:
            AI_speak("목적지에 중복된 이름의 directory가 없습니다")
